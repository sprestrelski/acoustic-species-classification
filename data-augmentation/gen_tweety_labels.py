...
def gen_labels(path):
    if not os.path.exists(path):#os.path.join(path, "train_audio")):
        print(f"Directory not found in path {path}", file=sys.stderr)
        sys.exit(1)
    audio_path = path#os.path.join(path, "train_audio")
    ...
    automated_df.to_csv(os.path.join(path, "132PeruXC_TweetyNet_Labels.csv"))
